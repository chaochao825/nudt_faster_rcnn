# Faster R-CNN 检测器 - 环境变量文档

## 概述
本文档描述了可用于配置 Faster R-CNN 检测器的所有环境变量，用于车辆识别任务，包括对抗样本生成、攻击评估、防御机制和模型训练。
...
data: {"status": "success", "message": "Input model file is valid and complete.", "file_name": "/project/input/model/fasterrcnn.pth"}
...
6. **模型架构**: Faster R-CNN 由于 torchvision 的限制默认使用 ResNet-50 配合 FPN，但遵循 VGG16 配置模式。
